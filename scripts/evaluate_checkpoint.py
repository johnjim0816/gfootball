...
def main(checkpoint, algorithm, env_name, config, n_episodes, debug):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--algorithm', type=str, default='PPO',
                        help="The algorithm or model to load. This may refer to the name\
                            ...
                            \nDefault: PPO.")
    parser.add_argument('-e','--n_episodes', default=10, type=int,
    ...
    assert args.n_episodes <= 100, "Must rollout a maximum of 100 episodes"
    assert args.n_episodes >= 1, "Must rollout at least 1 episode"
    ...
    for name, check in checkpoints.items():
        ...
        print("\tNumber of episodes to run (-e):", args.n_episodes)
        print("\tIs this a dry-run only (-r):", args.dry_run)
        print("\tScript running on debug mode (-d):", args.debug)
        print("")

        if not args.dry_run:
            output = main(args.checkpoint, args.algorithm, test_env_name, config, args.n_episodes, args.debug)
